...
const wss = new WebSocket.Server({
  ...
  verifyClient: (info) =>
    info.origin &&
    !!info.origin.match(
      /^https?:\/\/([^.]+\.github\.io|localhost|the-resistance-avalon\.boardgameonline\.dpdns\.org)/i,
    ),
})
...
wss.on('connection', function connection(ws, req) {
  // url pattern: the-resistance-avalon.boardgameonline.dpdns.org/<channel>/<playerId|host>
  ...
  if (
    ...
    channels[ws.channel].some(
      (client) =>
        client !== ws && client.readyState === WebSocket.OPEN && client.playerId === 'host',
    )
  ) {
